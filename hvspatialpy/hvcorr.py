...
def _get_locations(base_dir, site, xml_folder_name):
    ...
    site_dir = os.path.join(base_dir, str(site))
    ...
    for fi in range(num_peaks):
        for min_dist, max_dist in dist_range_list:
            ...
            for test in test_w_dist:
                ...
                if unique_flag:
                    hvsr_df = pd.read_csv(
                        os.path.join(base_dir, str(site), test, unique_folder_name, mean_file_name),
                        header=None)
                    hvsr_ref_df = pd.read_csv(os.path.join(base_dir, str(site), ref_test_name,
                                                           unique_folder_name, mean_file_name),
                                              header=None)

                else:
                    hvsr_df = pd.read_csv(
                        os.path.join(base_dir, str(site), test, mean_file_name), header=None)
                    hvsr_ref_df = pd.read_csv(
                        os.path.join(base_dir, str(site), ref_test_name, mean_file_name),
                        header=None)
                ...
                if len(hvsr_mean) == len(hvsr_mean_ref) and len(hvsr_mean) > 0:
                    if correlation_type == 'dtw':
                        corr = dtw(hvsr_mean_ref, hvsr_mean, global_constraint=global_constraint, sakoe_chiba_radius=sakoe_chiba_radius)
