...
def _update_colorbar(ax, cbar, scatter, correlation_type):
    """
...
def _plot_site_image(ax2, df_site, correlation_type='lcss', frequency_interval=None):
    ...
    # Filter data based on the frequency interval
    ...
    _update_colorbar(ax2, cbar, scatter, correlation_type=correlation_type)
...
def _update_dataframe(base_dir, site, num_peaks, min_freq, max_freq):
    ...
    site_dir_path = os.path.join(base_dir, str(site))
...
def update_correlations(base_dir, site, xml_folder_name, unique_folder_name, ref_test_name, mean_file_name,
                        ...
                        min_distance=1, global_constraint='sakoe_chiba', eps=0.75, sakoe_chiba_radius=10, dist_range_list=None):
    """
    ...
    """
    if unique_flag:
        ref_file_path = os.path.join(base_dir, str(site), ref_test_name, unique_folder_name, mean_file_name)
    else:
        ref_file_path = os.path.join(base_dir, str(site), ref_test_name, mean_file_name)

    ref = pd.read_csv(ref_file_path, header=None)

    # Ensure the DataFrame has the correct format and handle errors
    try:
        ref = ref[1:].astype(float)
    except ValueError as e:
        raise ValueError(f"DataFrame conversion error: {e}")

    # Filter out rows based on frequency threshold
    ref = ref[ref[0] > freq_trun]

    # Extract mean reference values
    ...
    def on_change(_):
        ...
        _plot_intervals(ax1=ax1, freq=freq, mean_ref=mean_ref, min_freq=updated_min_freq,
                        max_freq=updated_max_freq, freq_trun=0.1)
        updated_df = _update_dataframe(base_dir=base_dir, site=site, num_peaks=num_peaks,
                                       min_freq=updated_min_freq, max_freq=updated_max_freq)
        updated_df_all = compute_correlations(base_dir=base_dir, site=site, freq_df=updated_df,
        ...
        _plot_site_image(ax2=ax2, df_site=updated_df_all, correlation_type=correlation_type,
    ...
    df = _update_dataframe(base_dir, site, num_peaks, min_freq, max_freq)

    df_all = compute_correlations(base_dir, site, df, xml_folder_name, unique_folder_name, ref_test_name,
                                  mean_file_name, unique_flag=unique_flag, auto_dist_calc_flag=auto_dist_calc_flag,
                                  correlation_type=correlation_type, dist_meas_df=dist_meas_df,
                                  loc_df_manual=loc_df_manual, global_constraint=global_constraint,
                                  eps=eps, sakoe_chiba_radius=sakoe_chiba_radius, dist_range_list=dist_range_list)

    _plot_site_image(ax2, df_all, correlation_type=correlation_type, frequency_interval=frequency_interval)
